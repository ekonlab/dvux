...
And now let's create a bar chart comparing the situation by country in 1990 to 2015
...
Now we can use a different bar approach, bars clustered instead of stacked
...
And finally we can go back and tweak a little bit the dodge into fill position
...
Not it's easier to see which countries have improved (those with green bars bigger than red bars) and which have not (green > red)

However, looks like there must be a way to see this better... let's try to use a different chart.

```{r , fig.width=10, fig.height=5, warning=FALSE,, message= FALSE }


g8_bis = ggplot(sub_saharan_df,aes(year,rural_water_imp_per,group = country)) + geom_line() + theme_minimal() + facet_wrap(~country)  + ggtitle("Percentage of rural population in Sub-Saharan Africa countries with access to clean water from 1990 to 2015") +  xlab("years") + ylab("% of rural population with access to clean water")
